...
const processIncomingMessage = async (body) => {
  try {
    ...
    if (!fromPhone || !messageId) {
      console.warn(
        "[Webhook Error] Missing recipient number (from) or message ID in payload.",
      );
      return;
    }
    ...
    if (msg.type === "text") {
      ...
      if (interactive?.type === "button_reply") {
        ...
        if (replyTitle) {
          ...
          replyId &&
          !replyId.startsWith("opt_") &&
          !replyId.startsWith("row_")
        ) {
          messageText = replyId;
        } else if (replyDesc) {
      ...
      console.log(
        `[Webhook] Processing incoming interactive reply from ${fromPhone}: "${messageText}"`,
      );
      ...
      try {
        ...
        console.error(
          "[Webhook Media Error] Failed to process image:",
          mediaErr.message,
        );
      ...
      console.log(
        `[Webhook] Processing incoming document: ${originalFilename} with ID: ${mediaId}`,
      );

      try {
        const downloaded = await whatsappService.downloadWhatsAppMedia(
          mediaId,
          originalFilename,
        );
        const publicUrl = `${env.APP_URL}${downloaded.relativePath}`;
        ...
        console.error(
          "[Webhook Media Error] Failed to process document:",
          mediaErr.message,
        );
      ...
      console.log(
        `[Webhook] Processing incoming audio/voice with ID: ${mediaId}`,
      );
      ...
      try {
        ...
        console.error(
          "[Webhook Media Error] Failed to process audio:",
          mediaErr.message,
        );
      ...
      console.log(
        `[Webhook] Processing incoming location: Lat ${lat}, Lng ${lng}`,
      );
    ...
    console.log(
      `[Webhook] Parsed ${msg.type} message from ${fromPhone}: ${messageText}`,
    );
    ...
    if (isDuplicate) {
      console.log(
        `[Webhook] Duplicate message ID detected: ${messageId}. Skipping.`,
      );
      return;
    }
    ...
    try {
      ...
      console.warn(
        "[Webhook Warning] Failed to trigger read receipt or typing indicator:",
        statusErr.message,
      );
    }
...
const wiraHitMsg = async (req, res) => {
  try {
    console.log(
      "[WIRA Hit Msg API] Incoming request payload:",
    ...
    if (!rawPhone) {
      return res.status(400).json({
        ...
        success: false,
        message:
          "Error sending message: 'phone' (recipient number) is required.",
        data: null,
      });
    }
    ...
    if (hasOptions && extractedLinks.length > 0) {
      ...
      try {
        ...
        console.warn(
          `[WIRA Hit Msg Error] Failed interactive send (${interactiveErr.message}), falling back to text message.`,
        );
        formattedReply = whatsappService.formatWiraResponse(innerPayload, true);
        metaRes = await whatsappService.sendTextMessage(
          recipientPhone,
    ...
    const outgoingMessageId =
      metaRes?.messages?.[0]?.id || `wira_hit_${Date.now()}`;

    // Log outgoing message to DB
    await messageLogModel
      .logMessage({
      ...
      .catch((err) =>
        console.error("[WIRA Hit Msg DB Log Error]", err.message),
      );
  ...
  try {
    ...
    if (!to || !message) {
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: 'to' (phone number) and 'message' (text content) are required.",
      });
    }
    ...
    // Log outgoing message to DB
    const outgoingMessageId =
      result?.messages?.[0]?.id || `direct_${Date.now()}`;
    await messageLogModel
      .logMessage({
        whatsappNumber: to,
        messageId: outgoingMessageId,
      ...
      .catch((err) =>
        console.error("[Send Direct API DB Log Error]", err.message),
      );
